...
- Image Prompt Writer refactor: family-specific compiler prompts (Flux, Illustrious, Pony, SDXL booru, NovelAI, natural-language) replace the monolithic multi-family system prompt. Fast mode (~600–900 tokens) for routine CG; Premium mode adds cinematography few-shots for gallery unlocks and player-requested illustrations (Auto selects Premium when the illustration reason matches). Structured `<scene_state>` YAML replaces duplicated narration continuity. Agent Editor adds a Rewrite Quality control (Auto / Fast / Premium). Recommended LLMs: Fast — Gemini 2.5 Flash, GPT-4.1 mini, Haiku; Premium — Claude Sonnet 4, Gemini 2.5 Pro, GPT-4.1.
- Image Prompt Writer v2 (Flux Physical Compiler): Premium mode now runs a two-stage pipeline — Scene Visual Compiler (LLM extracts visual YAML) then Flux hybrid prompt writer (Block 1 cinematic sentence, Block 2 spatial composition, Block 3 comma render stack). Anti-sludge rules ban literary mood garnish; physicality and body geometry prioritized. Game art style moved to system style profile; `<target_image_model>` removed from user message for built-in prompts. Fast path uses enriched deterministic scene schema (environment geometry, pose hints, materials/lighting). Content-filter failures log explicit guidance. Agent Editor adds Scene Compile toggle (Premium LLM / Off).
- Image Prompt Writer v3 (Visual Compiler): Premium mode expands to a 4-stage pipeline — Scene Visual Compiler → Visual Saliency Reducer → Prompt Director (English Blocks 1–2 only) → deterministic static style inject (Block 3). Saliency filter drops low-importance narrative microdetails and compresses pose to max 3 anchors. Scene schema adds composition directives, explicit camera geometry, and structural avoid list. Agent Editor adds dual LLM connections (Scene Compiler + Prompt Director). Fast path uses deterministic saliency + single director call. Saliency output is English-only with NSFW contact preservation; director omits lighting/style tokens (static inject handles Block 3); genre/setting Cyrillic tokens map to English Flux tags.
- Image Prompt Writer v4 (Visual Prompt Compiler): Deterministic assembly path replaces final LLM prose for ComfyUI by default. Shared schemas (`SceneAST`, `VisualTokenBundle`, `ShotGraph`) plus token vocabulary drive a code-based Flux assembler with composition constraint engine and scene-derived negative prompts. Pipeline: Scene Compiler → Visual Token Extractor → Shot Director (graph, not prose) → TypeScript assembler. NSFW post-gates inject penetration tokens when scene AST or draft hints imply explicit contact. Agent Editor adds Prompt Assembly toggle (Deterministic v4 / LLM Prose v3) and optional Quality Scorer connection (stub). Phase 2 adapters for Illustrious, Pony, and SDXL booru; Phase 3 character bible, reference feature extractor, and cross-scene memory stubs included.
