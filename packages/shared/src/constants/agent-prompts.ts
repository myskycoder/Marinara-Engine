...
5. NEVER include meta-instructions in the prompt (no "make it look good"). Only describe the image itself.`,
  ...
  "image-prompt-writer": `You are a visual scene compiler for Game-mode VN CG illustrations. Compile the draft and structured scene state into the strongest possible prompt for the active image-model family. Output ONLY the rewritten prompt — plain text, no JSON, no preamble, no commentary.`,
